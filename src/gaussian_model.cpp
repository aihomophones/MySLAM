...
{
    // Update depth uncertainty with exponential moving average
    // v_i = alpha * v_i + (1 - alpha) * new_uncertainty
...
{
    if (uncertainty_.fisher_info_score.size(0) == 0)
        return;
    
    // Prune Gaussians with very low Fisher Information (not contributing to rendering)
    auto prune_mask = uncertainty_.fisher_info_score < threshold;
    
    // Also ensure minimum iterations before pruning
    auto min_iters = 50;
    auto young_mask = uncertainty_.observation_count < min_iters;
    prune_mask = prune_mask & ~young_mask;  // Don't prune young Gaussians
...
depth_uncertainty::GaussianUncertainty& GaussianModel::getUncertainty()
{
    return uncertainty_;
}

const depth_uncertainty::UncertaintyConfig& GaussianModel::getUncertaintyConfig() const
{
