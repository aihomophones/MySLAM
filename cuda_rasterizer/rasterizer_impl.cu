...
CudaRasterizer::ImageState CudaRasterizer::ImageState::fromChunk(char*& chunk, size_t N)
{
	...
	// Depth-Photo-SLAM: allocate depth output buffers
	...
	int* radii)
{
	...
	FORWARD::render(
		...
		geomState.means2D,
		feature_ptr,
		geomState.depths,           // Depth-Photo-SLAM: per-Gaussian depths
		geomState.conic_opacity,
	...
	if (out_depth_sq != nullptr) {
		cudaMemcpy(out_depth_sq, imgState.out_depth_sq, img_size, cudaMemcpyDeviceToDevice);
	}
...
void CudaRasterizer::Rasterizer::backwardWithDepth(
	const int P, int D, int M, int R,
	const float* background,
	const int width, int height,
	const float* means3D,
	...
	float* dL_drot)
{
	...
	// Allocate buffer for per-Gaussian depth gradients
	float* dL_ddepths = nullptr;
	cudaMalloc(&dL_ddepths, P * sizeof(float));
	cudaMemset(dL_ddepths, 0, P * sizeof(float));

	const float* color_ptr = (colors_precomp != nullptr) ? colors_precomp : geomState.rgb;
	
	// Render backward with depth gradients
	BACKWARD::renderWithDepth(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
	...
	// Preprocess backward with depth gradients
	const float* cov3D_ptr = (cov3D_precomp != nullptr) ? cov3D_precomp : geomState.cov3D;
	BACKWARD::preprocessWithDepth(P, D, M,
		(float3*)means3D,
		radii,
		shs,
		geomState.clamped,
