...
renderCUDA(
	...
	const float* __restrict__ depths,           // Depth-Photo-SLAM: per-Gaussian depth
	...
	float* __restrict__ out_median_depth)       // Depth-Photo-SLAM: median depth at T=0.5
{
	...
	__shared__ float collected_depths[BLOCK_SIZE];  // Depth-Photo-SLAM: shared depth buffer
	...
	// Depth-Photo-SLAM: Initialize depth accumulators
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_depths[block.thread_rank()] = depths[coll_id];  // Depth-Photo-SLAM
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Depth-Photo-SLAM: Accumulate depth using alpha-blending
			// D = Σ w_i * d_i where w_i = alpha_i * T_i
	...
	if (inside)
	{
		...
		// Depth-Photo-SLAM: Write depth outputs
		out_depth[pix_id] = D;
