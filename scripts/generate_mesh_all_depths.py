#!/usr/bin/env python3
"""
Generate mesh from ALL 2000 rendered depth frames (not just keyframes).
Uses the new all_depths folder from Photo-SLAM.
"""
...
def create_mesh(result_dir, dataset_path, output_path, voxel_size=0.02, depth_scale=6553.5, stride=1):
    result_dir = Path(result_dir)
    dataset_path = Path(dataset_path)
    output_path = Path(output_path)
    
    # Find all_depths folder
    all_depths_dirs = list(result_dir.glob('*_shutdown/all_depths'))
    if not all_depths_dirs:
        raise FileNotFoundError("No all_depths folder found!")
    
    depth_dir = all_depths_dirs[0]
    depth_files = {int(p.stem.split('_')[0]): p for p in depth_dir.glob('*_depth.png')}
    logger.info(f'Found {len(depth_files)} depth files in {depth_dir}')
    ...
    # Align trajectories
    common = sorted(set(est_poses.keys()) & set(gt_poses.keys()))[:500]  # Use first 500 for alignment
    ...
    integrated = 0
    for fid in sorted(depth_files.keys())[::stride]:
    ...
    logger.info(f'Integrated {integrated} frames total')
...
if __name__ == '__main__':
    ...
    create_mesh(args.result_dir, args.dataset_path, args.output, args.voxel_size, args.depth_scale, args.stride)
